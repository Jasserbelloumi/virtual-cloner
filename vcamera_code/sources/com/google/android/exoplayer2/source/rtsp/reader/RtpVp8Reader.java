    ...
    private boolean validateVp8Descriptor(ParsableByteArray parsableByteArray, int i) {
        ...
        if ((readUnsignedByte & 16) != 16 || (readUnsignedByte & 7) != 0) {
            if (this.gotFirstPacketOfVp8Frame) {
                ...
                formatInvariant = i < nextSequenceNumber ? Util.formatInvariant("Received RTP packet with unexpected sequence number. Expected: %d; received: %d. Dropping packet.", Integer.valueOf(nextSequenceNumber), Integer.valueOf(i)) : "RTP packet is not the start of a new VP8 partition, skipping.";
            }
