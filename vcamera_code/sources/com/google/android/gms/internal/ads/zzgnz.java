...
public final class zzgnz implements zzggt {
    ...
    public final byte[] zza(byte[] bArr, int i) throws GeneralSecurityException {
        if (i <= 16) {
            ...
            byte[] zzd = max * 16 == length ? zzgmv.zzd(bArr, (max - 1) * 16, this.zzb, 0, 16) : zzgmv.zzc(zzggg.zza(Arrays.copyOfRange(bArr, (max - 1) * 16, length)), this.zzc);
            byte[] bArr2 = new byte[16];
            for (int i2 = 0; i2 < max - 1; i2++) {
                bArr2 = zzb.doFinal(zzgmv.zzd(bArr2, 0, bArr, i2 * 16, 16));
            }
            return Arrays.copyOf(zzb.doFinal(zzgmv.zzc(zzd, bArr2)), i);
        }
